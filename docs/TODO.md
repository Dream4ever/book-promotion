...
统计明细弹窗模板已拆成 `AnalyticsDetailModal.vue`，页面只控制可见性和导出动作。

- [x] 新增 `AnalyticsDetailModal.vue`
- [x] 将统计明细表格、空状态和 footer 操作移出 `App.vue`
- [x] 保持导出明细、关闭弹窗行为不变
- [x] 执行 `npm test` 和 `npm run build`

验收点:

- [x] `App.vue` 不再直接渲染统计明细表格
- [x] 明细行展示字段和时间格式不变
- [x] 空明细状态和导出按钮行为不变
...
学校、书目、推广商和报备 tab 已复用 `EntityListToolbar.vue` 渲染搜索、创建、导入、导出、批量删除工具栏。

- [x] 新增实体列表工具栏组件
- [x] 支持可选导入按钮和已选导出按钮
- [x] 逐步接入学校、书目、推广商、报备 tab
- [x] 执行 `npm test` 和 `npm run build`

验收点:

- [x] 四个 tab 的工具栏交互不变
- [x] 各 tab 仍可自定义搜索 placeholder 和创建按钮文案
